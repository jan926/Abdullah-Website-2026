...
export default function HomePage() {
  ...
  const heroGames = useMemo(
    () => games.filter((g) => g.heroFeatured).slice(0, 6),
    [games],
  );
  const fallbackHeroGames = useMemo(() => games.slice(0, 6), [games]);
  const displayHeroGames = useMemo(
    () => (heroGames.length > 0 ? heroGames : fallbackHeroGames),
    [heroGames, fallbackHeroGames],
  );
  ...
  const totalGamesPages = useMemo(
    () => Math.max(1, Math.ceil(games.length / GAMES_PER_PAGE)),
    [games.length],
  );
  ...
  const gameOfTheDay = useMemo(
    () => games.filter((game) => game.gameOfTheDay).slice(0, 5),
    [games],
  );
  ...
  const prevSlide = () => {
    if (displayHeroGames.length === 0) return;
    setCurrentSlide(
      (prev) => (prev - 1 + displayHeroGames.length) % displayHeroGames.length,
    );
  };
  ...
  useEffect(() => {
    const refresh = async () => {
      try {
        const [loadedGames, loadedSettings, loadedCategories] =
          await Promise.all([
            loadGamesHome(),
            loadSiteSettings(),
            loadCategories(),
      ...
      <section className="relative h-[min(60vh,700px)] min-h-[320px] overflow-hidden">
        {activeHeroGame && (
          ...
          >
            <LazyImage
              src={
                activeHeroGame.heroMedia ||
                activeHeroGame.backgroundImage ||
                activeHeroGame.cover
              }
                          ...
                          <div className="absolute bottom-0 p-4 w-full">
                            <div className="flex items-center gap-1 text-yellow-400 mb-2">
                              <span className="text-sm font-bold">
                                ★ {getGameDisplayStats(game.id).rating}
                              </span>
                            </div>
                          </div>
                        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {games
              .slice(
                ...
                (gamesPage - 1) * GAMES_PER_PAGE + GAMES_PER_PAGE,
              )
              .map((game) => (
                <Link
                  key={game.id}
